...
void AL_DeformObject(chaowk* pChao) {
    ...
    for (i = 0; i < 40; i++) {
        ...
        if (attr & PART_ATTR_NODE) {
            ...
            float zhvPosX =
                div_ratio_h * lerp(pPosN->x, pPosH->x, ratio_h) + div_ratio_v * lerp(pPosN->x, pPosV->x, ratio_v);
            float zhvPosY =
                div_ratio_h * lerp(pPosN->y, pPosH->y, ratio_h) + div_ratio_v * lerp(pPosN->y, pPosV->y, ratio_v);
            float zhvPosZ =
                div_ratio_h * lerp(pPosN->z, pPosH->z, ratio_h) + div_ratio_v * lerp(pPosN->z, pPosV->z, ratio_v);
...

            {
                ...
                float zhvAngX =
                    div_ratio_h * lerp(pAngN->x, pAngH->x, ratio_h) + div_ratio_v * lerp(pAngN->x, pAngV->x, ratio_v);
                float zhvAngY =
                    div_ratio_h * lerp(pAngN->y, pAngH->y, ratio_h) + div_ratio_v * lerp(pAngN->y, pAngV->y, ratio_v);
                float zhvAngZ =
                    div_ratio_h * lerp(pAngN->z, pAngH->z, ratio_h) + div_ratio_v * lerp(pAngN->z, pAngV->z, ratio_v);
...
void AL_DeformObjectChild(chaowk* pChao) {
    ...
    for (i = 0; i < 40; i++) {
...

        {
            ...
            zhvPosX = div_ratio_h * lerp(pPosN->x, pPosH->x, ratio_h) + div_ratio_v * lerp(pPosN->x, pPosV->x, ratio_v);
            zhvPosY = div_ratio_h * lerp(pPosN->y, pPosH->y, ratio_h) + div_ratio_v * lerp(pPosN->y, pPosV->y, ratio_v);
            zhvPosZ = div_ratio_h * lerp(pPosN->z, pPosH->z, ratio_h) + div_ratio_v * lerp(pPosN->z, pPosV->z, ratio_v);
